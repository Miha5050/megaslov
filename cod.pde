...
String findComPort() {
  ...
  if (ports.length == 0) {
    //println("ОШИБКА: Нет доступных COM-портов!");
    //exit();
    p = 0;
    return "";
  }
  ...
  for (String port : ports) {
    if (port.contains("Arduino") || port.contains("ttyUSB") || port.contains("COM")) {
      println("Автоматически выбран порт: " + port);
      return port;
    }
  }

  println("Выбран первый доступный порт: " + ports[0]);
  return ports[0];
}
...
void initSerial() {
  ...
  try {
    ...
    println("Ошибка подключения к порту: " + e.getMessage());
    p = 0;
  //  exit();
  }
}
...
// ========== ГЛАВНЫЙ ЦИКЛ ==========
// УДАЛЕНО: Переменные для непрерывного движения
// final int MOVE_INTERVAL = 300;
// int lastMoveTime = 0;
// int currentMoveDirection = 0;
// boolean isMoving = false;

// ДОБАВЛЕНО: Флаг для предотвращения множественных нажатий
...
void draw() {
  ...
  // УДАЛЕНО: Обработка непрерывного движения
  // if (isMoving && millis() - lastMoveTime >= config.MOVE_INTERVAL && inMainMenu == false) {
  //   handleContinuousMove();
  // }

  // ДОБАВЛЕНО: Обработка клавиатуры с защитой от множественных нажатий
  if (keyProcessed) {
    handleKeyboardInput();
  }
...
void handleKeyboardInput() {
  ...
  if (keyUpPressed && keyProcessed) {
    inputValue = 1;
    keyPressed = true;
  } else if (keyDownPressed && keyProcessed) {
    inputValue = 2;
    keyPressed = true;
  } else if (keyLeftPressed && keyProcessed) {
    inputValue = 3;
    keyPressed = true;
  } else if (keyRightPressed && keyProcessed) {
    inputValue = 4;
    keyPressed = true;
  } else if (keyEnterPressed && keyProcessed) {
    inputValue = 11;
    keyPressed = true;
  }
  ...
  if (keyPressed) {
    ...
    keyProcessed = false; // Блокируем дальнейшую обработку до отпускания клавиши
    
    // Сбрасываем флаги клавиш после обработки
    if (inputValue == 11) {
...
// ДОБАВЛЕНО: Обновление курсора меню с задержкой
void updateMenuCursor() {
  if (millis() - lastCursorMoveTime > config.CURSOR_DELAY) {
    if (pendingDirectionY != 0) {
      moveMenuCursor(pendingDirectionY);
...
void processInput(int value) {
  if (value == 0) {
    // ДОБАВЛЕНО: Сброс непрерывного движения при получении 0
    c = 1;
    isMoving = false;
    currentMoveDirection = 0;
    inputProcessed = true;
...
// ИСПРАВЛЕНО: Система перемещения курсора в меню (убрана переменная c)
void handleMenuInput(int value) {
  switch(value) {
  case 1:
    pendingDirectionY = -1;  // Вверх
    break;
  case 2:
    pendingDirectionY = 1;   // Вниз
    break;
  case 11:
...
void keyReleased() {
  ...
  case LEFT:
    keyLeftPressed = false;
    // Сброс движения только если это было последнее направление
